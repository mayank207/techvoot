...
(function (factory) {
    ...
    $h = {
        ...
        closeButton: function (css) {
            css = ($h.isBs(5) ? '' : 'close') + (css ? ' ' + css : '');
            return '<button type="text" class="' + css + '" aria-label="Close">\n' +
    ...
    FileInput.prototype = {
        ...
        _maximizeZoomDialog: function () {
            ...
            if ($head && $head.length) {
                h -= $head.outerHeight(false);
            }
            if ($foot && $foot.length) {
                h -= $foot.outerHeight(false);
            }
            if ($body && $body.length) {
                diff = $body.outerHeight(false) - $body.height();
                h -= diff;
            }
            $modal.find('.kv-zoom-body').height(auto);
        },
            ...
            if ($modal.hasClass('file-zoom-fullscreen')) {
                ...
                if (!fullScreen) {
                    if (!$btnFull.hasClass('active')) {
                        ...
                        self.$modal.find('.kv-zoom-body').css('height', 'auto');
                    } else {
                        $btnFull.removeClass('active').attr('aria-pressed', 'false');
                    }
                } else {
